...
@IdHolder(ids = {72, 88, 21, 94, 73, 30})
public class NPCActionHandler implements Incoming {
    ...
    public void handle(Player player, InBuffer in, int opcode) {
        ...
        if(option == 1) {
            int targetIndex = in.readUnsignedShortA();
            int ctrlRun = in.readByteC();
            handleAction(player, option, targetIndex, ctrlRun);
            return;
        }
        if(option == 2) {
            int targetIndex = in.readUnsignedLEShortA();
        ...
        if(option == 6) {
            ...
            if(def == null)
                return;
            player.sendMessage(def.name); //todo examine
            if(player.debug)
