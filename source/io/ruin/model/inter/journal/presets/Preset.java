...
public class Preset extends JournalEntry {
    ...
    public final boolean allowSelect(Player player) {
        ...
        if(World.isPVP() && player.presetDelay.isDelayed()) {
        ...
        if(player.getPosition().inBounds(FFAClanWars.FFA_BOUNDS)) {
            return true;
        }
        if (!player.getPosition().inBounds(EDGE_BOUNDS) && !player.getPosition().inBounds(MAGE_BANK)) {
            ...
            player.dialogue(new MessageDialogue("Presets may only be used at Edgeville."));
            return false;
        }
        if(World.isEco()/* && !player.isPKMode() && this.free*/) {
    ...
    private void withdrawItems(Player player, boolean inventory) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if(World.isPVP() && presetItem.getId() == RingOfRecoil.RING_OF_RECOIL)
    ...
    static {
        ...
        int[] presetItems = new int[]{
                ...
                EARTH_RUNE, MAGIC_SHORTBOW, RUNE_CROSSBOW, DRAGON_SCIMITAR, ANCIENT_STAFF, DRAGON_DAGGER, RUNE_ARROW,
