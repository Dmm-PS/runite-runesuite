...
public class TabPrayer {
        ...
        InterfaceHandler.register(Interface.PRAYER, h -> {
            for(Prayer prayer : Prayer.values()) {
                if(prayer == Prayer.RIGOUR) {
                    h.actions[prayer.ordinal() + 4] = (DefaultAction) (p, option, slot, itemId) -> {
                        if(option == 1)
                    ...
                    h.actions[prayer.ordinal() + 4] = (SimpleAction) p -> p.getPrayer().toggle(prayer);
                }
            }
        });
    ...
    public static void refundPrayer(Player player, Prayer prayer) {
        if(prayer == Prayer.AUGURY && Config.AUGURY_UNLOCK.get(player) == 0) {
            player.dialogue(new MessageDialogue("You have to learn how to use " + Color.COOL_BLUE.wrap("Augury") + " before attempting to refund the scroll!"));
            return;
        }

        if(prayer == Prayer.RIGOUR && Config.RIGOUR_UNLOCK.get(player) == 0) {
            player.dialogue(new MessageDialogue("You have to learn how to use " + Color.COOL_BLUE.wrap("Rigour")+ " before attempting to refund the scroll!"));
            return;
        }
        ...
        int cost = World.isPVP() ? 5000 : 5000000;
