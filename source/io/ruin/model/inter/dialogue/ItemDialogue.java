...
public class ItemDialogue implements Dialogue {
    ...
    public void open(Player player) {
        if(itemId2 == -1) {
            ...
            player.getPacketSender().setHidden(Interface.ONE_ITEM_DIALOGUE, 5, true);
            if(hideContinue)
                player.getPacketSender().setHidden(Interface.ONE_ITEM_DIALOGUE, 3, true);
            else {
                player.getPacketSender().setHidden(Interface.ONE_ITEM_DIALOGUE, 3, false);
                player.getPacketSender().sendString(Interface.ONE_ITEM_DIALOGUE, 3, "Click here to continue");
            }
        } else {
    ...
    static {
        InterfaceHandler.register(Interface.ONE_ITEM_DIALOGUE, h -> h.actions[3] = (SimpleAction) Player::continueDialogue);
