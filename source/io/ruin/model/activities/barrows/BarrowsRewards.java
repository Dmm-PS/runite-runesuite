...
public class BarrowsRewards {
    ...
    private static final LootTable miscTable = new LootTable()
            ...
            .addTable(1, //other
                    ...
                    new LootItem(995, 10000, 250000, 1)     // coins
            );
    ...
    public static ItemContainer loot(Player player) {
        ...
        double customBarrowsChance = barrowsChance * (World.isPVP() ? 5 : 2);
        ...
        if(World.isPVP() && brothersKilled == 6) {
            int amount = Random.get(100, 400);
            container.add(13307, amount);
            player.sendMessage(Color.COOL_BLUE.wrap("You receive an additional " + amount + " blood money for killing all 6 brothers."));
        }
        ...
        for(int i = 0; i < brothersKilled; i++) {
            if(Random.get() > customBarrowsChance) {
                 ...
                 */
                Item item;
                if(!miscUnique && (item = miscUniqueTable.rollItem()) != null && World.isEco()) {
                    miscUnique = true;
                } else {
                    item = miscTable.rollItem();
                    if(item.getId() == 995 && World.isPVP())
                        item = new Item(13307, Random.get(200, 400));
