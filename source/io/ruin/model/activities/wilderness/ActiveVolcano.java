...
public class ActiveVolcano {
    ...
    private static final int BLOOD_FRAGMENT = 21532;
    private static int BOULDER_STABILITY = 10000;
    ...
    private static boolean checkActive(Player player) {
        ...
        if (active) {
            ...
            player.getPacketSender().sendString(Interface.VOLCANO_MINE, 13, boulderColor() + (int) (BOULDER_STABILITY * .01) + "%");
    ...
    private static String boulderColor() {
        if (BOULDER_STABILITY > 6000)
            return "<col=00FF00>";
        else if (BOULDER_STABILITY > 2000)
            return "<col=ffff00>";
        return "<col=FF0000>";
    }
    ...
    public static final class Entry extends JournalEntry {
        ...
        public void send(Player player) {
            ...
            if (minsLeft == 0)
                send(player, "Active Volcano", "Right away!", Color.ORANGE_RED);
            else if (minsLeft == 1)
                send(player, "Active Volcano", "1 minute", Color.GREEN);
            else if (minsLeft == 60)
                send(player, "Active Volcano", "1 hour", Color.GREEN);
            else if (minsLeft > 60) {
                int mins = minsLeft - 60;
                send(player, "Active Volcano", "1 hour " + mins + " minute" + (mins > 1 ? "s" : ""), Color.GREEN);
            } else
                send(player, "Active Volcano", minsLeft + " minutes", Color.GREEN);
        }
