...
public enum Volcano {
    ...
    private static void forgeRing(Player player, Volcano shield) {
    ...
    static {
        ...
        for (Volcano shield : Volcano.values()) {
            ItemObjectAction.register(shield.firstShard.getId(), 26755, (player, item, obj) -> forgeRing(player, shield));
            ItemObjectAction.register(shield.secondShard.getId(), 26755, (player, item, obj) -> forgeRing(player, shield));
            ItemObjectAction.register(shield.thirdShard.getId(), 26755, (player, item, obj) -> forgeRing(player, shield));
        }
