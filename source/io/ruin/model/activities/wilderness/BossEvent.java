...
public class BossEvent extends JournalEntry {
    ...
    private static final double BLOOD_MONEY_PER_DAMAGE = World.isPVP() ? 4 : 300;
    ...
    private static final LootTable LOOT_TABLE = new LootTable().addTable(1,
            new LootItem(13307, 2000, 5000, 20),   // Blood money
            ...
            new LootItem(6920, 1, 1),             // Infinity boots
            new LootItem(13237, 1, 1),            // Pegasian boots
            new LootItem(13239, 1, 1),            // Primordial boots
            ...
            new LootItem(11832, 1, 1),            // Bandos chestplate
    ...
    private static final LootTable ECO_LOOT_TABLE = new LootTable().addTable(1,
            new LootItem(995, 250000, 500000, 20),   // Blood money
            ...
            new LootItem(19478, 1, 5),            // Heavy ballista
            new LootItem(11802, 1, 1),            // Armadyl godsword
            new LootItem(11804, 1, 1),            // Bandos godsword
            new LootItem(13271, 1, 1),            // Abyssal dagger
            new LootItem(11283, 1, 1),            // Dragon fireshield
    ...
    public void send(Player player) {
        ...
        if (minsLeft == 0)
            send(player, bossName, "less than a minute!", Color.ORANGE_RED);
        else if (minsLeft == 1)
            send(player, bossName, "1 minute", Color.GREEN);
        else if (minsLeft == 60)
            send(player, bossName, "1 hour", Color.GREEN);
        else if (minsLeft > 60) {
            int mins = minsLeft - 60;
            send(player, bossName, "1 hour " + mins + " minute" + (mins > 1 ? "s" : ""), Color.GREEN);
        } else
            send(player, bossName, minsLeft + " minutes", Color.GREEN);
    }
    ...
    private static NPC spawnNext() {
        ...
        World.players.forEach(player -> player.getPacketSender().sendMessage(spawnMessage, "", 14));
    ...
    private static void rewards(NPC boss, Killer topKiller) {
        ...
        String currencyName = World.isPVP() ? "blood money" : "coins";
        for (Killer k : boss.getCombat().killers.values()) {
            ...
            if (player != null && player.getPosition().isWithinDistance(boss.getPosition(), 20)) {
                /* blood money reward for damaging */
                int amount = (int) ((topKiller.damage * (World.isPVP() ? bloodMoneyPerDamage(player) : goldMoneyPerDamage(player))) + (topKiller.player.wildernessLevel * BLOOD_MONEY_PER_WILD_LEVEL));
                player.getInventory().addOrDrop(World.isPVP() ? 13307 : 995, amount);
                if (k == topKiller) { // whoever dealt most damage
                    ...
                    if (pet != null && Random.rollDie(pet.dropAverage))
                        pet.unlock(player);
                    //^ Remember, top killer will roll for pet elsewhere.
                }
            }
        }
    }
    ...
    static {
        World.startEvent(e -> {
            ...
            while (true) {
                NPC npc = spawnNext();
                nextSpawn = Server.getEnd(30 * 100);
