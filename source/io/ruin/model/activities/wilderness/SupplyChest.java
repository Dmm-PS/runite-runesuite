...
public class SupplyChest {
    ...
    private static final SupplyChest[] SPAWN_LOCATIONS = {
            ...
            new SupplyChest(new Position(3034, 3683, 0)),
            new SupplyChest(new Position(3279, 3879, 0)),
            new SupplyChest(new Position(2982, 3910, 0)),
            new SupplyChest(new Position(3022, 3862, 0))
    };

    private static final String[] LOCATION_HINTS = {
            "east of the Mage Arena entrance",
            "at the Bandit Camp",
            "at the Demonic Ruins",
            "west of the Agility Training Area",
            "north east of the KBD entrance"
    };

    /* loot table */
    public static final LootTable SUPPLY_CHEST_TABLE = new LootTable().addTable(1,
            new LootItem(13307, 2500, 10000, 35), // Blood money
            new LootItem(11802, 1, 1), // Armadyl godsword
            new LootItem(11806, 1, 1), // Saradomin godsword
            new LootItem(11808, 1, 1), // Zamorak godsword
            new LootItem(11832, 1, 1), // Bandos tassets
            new LootItem(11834, 1, 1), // Bandos chestplate
    ...
    private static void lootChest(Player player, GameObject obj) {
        ...
        if(World.isPVP() && brewCount > 13) {
            player.sendMessage("You can't open the chest with more than 13 brews!");
            return;
        }
        player.startEvent(event -> {
            if(looted) {
    ...
    public static final class Entry extends JournalEntry {
        ...
        public void send(Player player) {
            ...
            if (minsLeft == 0)
                send(player, "PvP Supply Chest", "Right away!", Color.ORANGE_RED);
            else if (minsLeft == 1)
                send(player, "PvP Supply Chest", "1 minute", Color.GREEN);
            else if (minsLeft == 60)
                send(player, "PvP Supply Chest", "1 hour", Color.GREEN);
            else if (minsLeft > 60) {
                int hours = minsLeft / 60;
                send(player, "PvP Supply Chest", hours + " hour" + (hours > 1 ? "s" : ""), Color.GREEN);
            } else
                send(player, "PvP Supply Chest", minsLeft + " minutes", Color.GREEN);
        }
