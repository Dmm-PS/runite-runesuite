...
public class ChambersOfXeric {
    ...
    static {
        ...
        LoginListener.register(p -> { //attempt to rejoin if logged out during a raid
            if (p.partyRejoinId == -1)
                return;
            for (Player foundPlayer : World.players) { //not ideal, but very uncommon so no big deal
                if (foundPlayer.raidsParty != null && foundPlayer.raidsParty.getPartyId() == p.partyRejoinId && foundPlayer.raidsParty.getRaid() != null) {
                    foundPlayer.raidsParty.getRaid().rejoinRaid(p);
                    return;
                }
            }
            p.partyRejoinId = -1;
            p.sendMessage(Color.RAID_PURPLE.wrap("Could not rejoin your raid party."));
        });
    ...
    private Chamber[][][] chambers = new Chamber[2][4][2]; // zxy; 2 floors here since the final one is fixed
    ...
    public void generate() {
        ...
        for (int floorNumber = 0; floorNumber < 2; floorNumber++) {
            ...
            floor[x][y] = floorNumber == 1 ? START.newChamber() : LOWER_LEVEL_START.newChamber();
            if (floorNumber == 1)
            ...
            while (true) { // add random rooms
                ...
                if (nextMove == null || randomRooms == 6 && rng.nextDouble() > SIXTH_ROOM_CHANCE) { // no moves left, or made 5 rooms and failed roll for 6th
                    ...
                    else if (floorNumber == 1 && scavengerBudget > 0 && rng.nextDouble() < 0.8) {
    ...
    private void enterRaid(Player player) {
        ...
        player.startEvent(event -> {
            ...
            if (party.getMembers().stream().allMatch(ChambersOfXeric::isRaiding)) {
                party.getLeader().sendMessage(Color.RAID_PURPLE.wrap("All party members have entered the raid!"));
            }
            event.delay(2);
            player.unlock();
        });
    ...
    private void rejoinRaid(Player player) {
        player.raidsParty = party;
        player.getPacketSender().fadeIn();
        party.addMember(player);
        player.getMovement().teleport(getStartingChamber().getEntrancePosition());
        entered(player);
        assignListener(player);
        party.getMembers().forEach(p -> {
            p.sendMessage(Color.RAID_PURPLE.wrap(player.getName() + " has reconnected."));
            Party.sendRaidJournal(p, party);
        });
        player.partyRejoinId = -1;
    }
    ...
    private void exited(Player player, boolean logout) {
        ...
        if (logout) {
            if (isStarted()) {
                player.partyRejoinId = party.getPartyId();
            }
            getParty().getMembers().forEach(p -> p.sendMessage(Color.RAID_PURPLE.wrap(player.getName() + " has disconnected.")));
    ...
    private void handleDeath(Player player) {
        ...
        player.getPacketSender().resetCamera();
        int pointsLost = (int) ((Config.RAIDS_PERSONAL_POINTS.get(player) * 0.4) * getPlayerPointLossPercentage(player));
        if (pointsLost > 0)
            addPoints(player, -pointsLost);
    }

    private double getPlayerPointLossPercentage(Player player) {
        if (player.isGroup(PlayerGroup.GODLIKE_DONATOR)) {
            return 0.75;
        }
       // } else if (player.isGroup(PlayerGroup.MAS))

        return 1; // 100%
    }

    public int getRaidStageConfig() {
        switch (raidStage) {
            case 0:
