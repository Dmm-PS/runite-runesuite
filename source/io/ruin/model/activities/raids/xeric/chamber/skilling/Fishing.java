...
public enum Fishing {
    ...
    private static void fish(Player player, Fishing fish) {
        ...
        if(!player.getInventory().hasId(FISHING_ROD)) {
            player.sendFilteredMessage("You need a fishing rod to fish from this spot.");
            return;
        }

        if(!player.getInventory().hasId(CAVE_WORMS)) {
            player.sendFilteredMessage("You need at least one cave worm to fish from this spot.");
            return;
        }
        ...
        player.startEvent(event -> {
            ...
            while(true) {
                ...
                if(success(player, fish.levelReq)) {
                    ...
                    if(!player.getInventory().hasId(CAVE_WORMS)) {
                        player.sendFilteredMessage("You've ran out of bait!");
