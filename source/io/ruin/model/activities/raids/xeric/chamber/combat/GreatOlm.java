...
public class GreatOlm extends NPCCombat {
    ...
    public boolean attack() {
        ...
        if (attackType == 0 || attackType == 2) {
            if (finalStand && !siphonDelay.isDelayed() && Random.rollDie(5)) {
                siphonAttack();
                siphonDelay.delay(20);
            } else {
                PhasePower power = isEmpowered() ? Random.get(PhasePower.values()) : phasePower;
                if (power == PhasePower.ACID && Random.rollPercent(30)) {
                    if (Random.rollPercent(50))
                        acidPoolsAttack();
                    else
                        acidDrip();
                } else if (power == PhasePower.FLAME && Random.rollPercent(30)) {
                    if (Random.rollPercent(50))
                        flameWall();
                    else
                        burnAttack();
                } else if (power == PhasePower.CRYSTAL && Random.rollPercent(30)) {
                    if (Random.rollPercent(50))
                        crystalBomb();
                    else
                        crystalMark();
        ...
        for (int i = 0; i < 4; i++) {
            ...
            npc.addEvent(event -> {
                ...
                if (distance == 0) {
                    ...
                    player.hit(new Hit(npc).fixedDamage(distance * 5));
                    if (finalOther != null) {
                        finalOther.getMovement().teleport(player.getPosition());
                        finalOther.hit(new Hit(npc).fixedDamage(distance * 5));
                    }
                }
            });
        }
        ...
        for (int i = 0; i < bombCount; i++) {
            npc.addEvent(event -> {
                ...
                forAllTargets(p -> {
                    ...
                    if (distance > 3)
                        return;
                    p.hit(new Hit(npc).fixedDamage(60 - (distance * 15)));
                });
            });
        }
    }
    ...
    private void flameWall() {
        ...
        npc.addEvent(event -> {
            ...
            event.delay(1);
            List<GameObject> fires = new ArrayList<>(20);
            if (isOnEastSide()) {
                for (int x = projectileX; x < projectileX + 10; x++) {
    ...
    private void siphonAttack() {
        ...
        npc.addEvent(event -> {
            ...
            for (Player player : getAllTargets()) {
                ...
                if (!safe) {
                    damageDealt += player.hit(new Hit(npc).randDamage(10, 15));
                }
            }
            npc.hit(new Hit(HitType.HEAL).fixedDamage(damageDealt * 5));
        });
    }
