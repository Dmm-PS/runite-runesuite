...
public class RecruitingBoard {
    ...
    public static List<Party> ADVERTISED_PARTIES = new ArrayList<>(50);
    ...
    static {
        ...
        InterfaceHandler.register(Interface.RAIDING_RECRUITING_BOARD, h -> {
            ...
            h.actions[14] = (DefaultAction) (player, option, slot, itemId) -> {
                if (slot < ADVERTISED_PARTIES.size() && ADVERTISED_PARTIES.get(slot) != null) {
                    displayParty(player, ADVERTISED_PARTIES.get(slot));
                }
            };
        });
        InterfaceHandler.register(Interface.RAIDING_PARTY, h -> {
            h.actions[3] = (DefaultAction) (player, option, slot, itemId) -> {
                if (slot == 0)
                    if (player.raidsParty == null)
                        ...
                        leaveParty(player);
                if (slot == 1)
                    advertiseParty(player);
                if (slot == 2)
                    disbandParty(player);
                if (slot == 3) {
                    displayParty(player, player.viewingParty != null ? player.viewingParty : player.raidsParty);
                }
                if (slot == 4)
                    displayRecruitingBoard(player);
                if (slot == 5)
            ...
            h.closedAction = (player, integer) -> {
                ...
                if (player.raidsParty != null)
                    copyPartyPreferences(player, player.raidsParty);
            };
        });
    }
    ...
    private static void displayRecruitingBoard(Player player) {
        ...
        for (int i = 0; i < 40; i++) {
            if (i < ADVERTISED_PARTIES.size() && ADVERTISED_PARTIES.get(i) != null) {
                Party party = ADVERTISED_PARTIES.get(i);
                Player partyHost = party.getLeader();
                String partyInformation = "<col=ffffff>" + partyHost.getName() + "</col>" + "|" +
                        ...
                        "0|" + // challenge mode
                        (Server.currentTick() - party.getAdvertisementStartTick()) + "|";
    ...
    public static void joinParty(Player player, Party party) {
        if (!ADVERTISED_PARTIES.contains(party)) {
            player.dialogue(new MessageDialogue("This party is no longer recruiting."));
            return;
        }
        if (party.getRaid() != null && party.getRaid().isStarted()) {
            player.dialogue(new MessageDialogue("This party has already started its raid."));
            return;
        }
        ...
        party.getLeader().dialogue(new OptionsDialogue(player.getName() + " (" + player.getCombat().getLevel() + " combat, " + NumberUtils.formatNumber(player.getStats().totalLevel) + " total level) wishes to join your party.",
                ...
                new Option("Decline", () -> {
                    player.dialogue(new MessageDialogue(party.getLeader().getName() + " has declined your request to join their raid party."));
    ...
    public static void disbandParty(Player player) {
        ...
        if (player.raidsParty.getLeader() == player) {
            ...
            player.raidsParty.getMembers().forEach(member -> {
                ...
                member.sendMessage("<col=ef20ff>" + player.getName() + " has disbanded the party.");
            ...
            ADVERTISED_PARTIES.remove(player.raidsParty);
    ...
    private static void advertiseParty(Player player) {
        if (player.raidsParty == null)
            return;
        boolean delist = player.raidsParty.isAdvertising();
        if (delist)
            ADVERTISED_PARTIES.remove(player.raidsParty);
        else
            ADVERTISED_PARTIES.add(player.raidsParty);
        player.raidsParty.setAdvertising(!delist);
        player.raidsParty.setAdvertisementStartTick((int) (delist ? 0L : Server.currentTick()));
        displayParty(player, player.raidsParty);
    }
