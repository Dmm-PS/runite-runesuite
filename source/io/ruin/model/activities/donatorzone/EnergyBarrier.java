...
public class EnergyBarrier {
    ...
    static {
        ObjectAction.register(DONATOR_ENTRANCE, 1, (player, obj) -> {
             ...
             */
            if(player.isDonator() || player.isSupport() || player.isModerator() || player.isAdmin()) {
                if(player.getAbsY() <= 3507)
                    enter(player, obj);
                 else
                    exit(player, obj);
            } else {
                player.dialogue(new ItemDialogue().one(13307, "You need to be a " + Color.COOL_BLUE.wrap("donator") +
