...
public class FFAClanWars {
    ...
    static {
        ...
        ObjectAction.register(EXIT_PORTAL, "exit", (player, obj) -> {
            if (World.isPVP()) {
                player.getMovement().teleport(3092, 3504, 0);
            } else {
                player.getMovement().teleport(3352, 3163, 0);
            }
        });
        if(World.isPVP())
            Tile.get(3091, 3506, 0, true).flagUnmovable();
