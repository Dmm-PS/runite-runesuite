...
public class GodwarsObstacle {
    ...
    static {
        ...
        ObjectAction.register(26518, 2885, 5344, 2, 1, (player, obj) -> {
            if (player.getHp() < 70 || player.getStats().get(StatType.Hitpoints).fixedLevel < 70 && World.isEco()) {
        ...
        ObjectAction.register(26561, 2913, 5300, 2, 2, (player, obj) -> {
            if (!player.getStats().check(StatType.Agility, 70, "use this shortcut") && World.isEco())
                return;
            player.startEvent(event -> {
                player.lock();
                player.animate(828);
