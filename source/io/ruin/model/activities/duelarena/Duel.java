...
public class Duel extends ItemContainer {
    ...
    public void request(Player target) {
        ...
        if(targetDuel.requestUserId == player.getUserId()) {
            if(!target.getPosition().inBounds(DuelArena.BOUNDS)) {
                player.sendMessage("Your target isn't inside the duel arena.");
                return;
            }
            if(!player.getPosition().inBounds(DuelArena.BOUNDS)) {
    ...
    private void offer(Item item, int amount) {
        ...
        if(!item.getDef().tradeable && item.getId() != 13307) {
    ...
    private void second() {
        ...
        if(World.isPVP()) {
            player.getPacketSender().setHidden(704, 17, true);
            player.getPacketSender().setHidden(704, 25, true);
        }
    ...
    public void handleDraw() {
        ...
        player.getMovement().teleport(END_BOUNDS.randomX(), END_BOUNDS.randomY(), 0);
