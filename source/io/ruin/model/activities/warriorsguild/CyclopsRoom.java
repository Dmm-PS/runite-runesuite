...
public class CyclopsRoom {
    ...
    private static void checkTokenEvent(Player player) {
        ...
        if (tokens != null) {
            ...
            if (player.isDonator() && !player.isGroup(PlayerGroup.DONATOR) && Random.get() <= 0.10) {
                player.sendMessage("<col=1e44b3>Your donator status comes in handy and prevents your tokens from crumbling away!");
                return;
            }
