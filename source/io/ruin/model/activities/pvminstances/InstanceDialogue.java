...
public class InstanceDialogue {
    ...
    private void confirmCreation() {
        ...
        Item cost = new Item(World.isEco() ? 995 : 13307, type.getCost());
    ...
    private void create() {
        Item cost = new Item(World.isEco() ? 995 : 13307, (int) (type.getCost() * getCostModifier(player)));
    ...
    private double getCostModifier(Player player) {
        if (player.isGroup(PlayerGroup.GODLIKE_DONATOR)) {
            return 0.75;
        } else if (player.isGroup(PlayerGroup.UBER_DONATOR) || player.isGroup(PlayerGroup.LEGENDARY_DONATOR)) {
            return 0.80;
    ...
    private Option toOption(InstanceType instanceType) {
        return new Option(instanceType.getName() + " - " +
                "" + NumberUtils.formatNumber(instanceType.getCost()) + " " + (World.isEco() ? "Coins" : "Blood money") + "",
//                "Duration: " + (instanceType.getDuration() / 100) + " minutes",
                () -> {
                    type = instanceType;
                    createMenu();
                });
