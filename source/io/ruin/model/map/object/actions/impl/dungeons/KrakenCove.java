...
public class KrakenCove {
    ...
    static {
        SpawnListener.forEach(n -> {
            if (n.getCombat() != null && n.spawnPosition.getRegion().id == 9116) {
                n.attackNpcListener = (player, npc, message) -> {
                    if (!Slayer.isTask(player, npc) && !World.isPVP()) {
                        player.sendMessage("" + player.slayerTask.name);
                        if(player.slayerTask.name.equalsIgnoreCase("cave kraken") && npc.getId() == 5534 || npc.getId() == 5535)
                            return true;
                        if (message)
        ...
        ObjectAction.register(538,"use", (player, obj) -> {
            player.getMovement().teleport(2280, 10016);
        });
    }
}
