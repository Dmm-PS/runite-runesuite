...
public class TomeOfFire {
    ...
    private static void pagesToRemove(Player player, Item primary) {
        player.dialogue(new OptionsDialogue(
                ...
                new Option("Remove X pages.", () -> player.integerInput("Remove how many pages? (0-" + primary.getUniqueValue() / 20 + ")", pagesToAdd -> removePages(player, primary, pagesToAdd, false))),
    ...
    private static void addPages(Player player, Item tomeOfFire) {
        int charges = tomeOfFire.getUniqueValue();
        ...
        tomeOfFire.setUniqueValue(charges + addAmount);
    ...
    private static void removePages(Player player, Item tomeOfFire, int pagesToRemove, boolean emptyBook) {
        ...
        while (pagesToRemove-- > 0) {
            if (tomeOfFire.getUniqueValue() < 20)
                break;
            tomeOfFire.setUniqueValue(tomeOfFire.getUniqueValue() - 20);
            player.getInventory().add(BURNT_PAGE, 1);
            if (tomeOfFire.getUniqueValue() <= 0)
