...
public enum DragonfireShield {
    ...
    static {
        for(DragonfireShield shield : DragonfireShield.values()) {
            ...
            /**
             * Using blood money on the dragonfire shields
             */
            ItemItemAction.register(shield.chargedId, BLOOD_MONEY, DragonfireShield::payToCharge);
            ItemItemAction.register(shield.unchargedId, BLOOD_MONEY, DragonfireShield::payToCharge);
        }
    }
    ...
    private static void check(Player player, Item shield) {
        ...
        if(World.isPVP() && charges != MAX_CHARGES)
        player.sendMessage("<col=007f00>You can charge this by consuming the breathe of a dragon or by using blood money on it.");
    }
    ...
    private static void payToCharge(Player player, Item shield, Item bloodMoney) {
        ...
        player.dialogue(new YesNoDialogue("Are you sure you want to do this?", "Pay 2,500 blood money to set the shields charges to 50?", shield, () -> {
            if(bloodMoney.getAmount() < 2500) {
                player.dialogue(new ItemDialogue().one(13307, "You need at least " + Color.DARK_RED.wrap("2,500 blood money") + " to do this!"));
                return;
            }
