...
public class SlayerHelm {
    ...
    private static final List<Integer> MELEE_BOOST_HELMS = Arrays.asList(
            ...
            SLAYER_HELM, RED_SLAYER_HELM, GREEN_SLAYER_HELM, BLACK_SLAYER_HELM, PURPLE_SLAYER_HELM, TURQUOISE_SLAYER_HELM
    );

    private static final List<Integer> ALL_BOOST_HELMS = Arrays.asList(
            8901, 8903, 8905, 8907, 8909, 8911, 8913, 8915, 8917, 8919, 8921, // black masks
            SLAYER_HELM_IMBUE, RED_HELM_IMBUE, GREEN_HELM_IMBUE, BLACK_HELM_IMBUE, PURPLE_HELM_IMBUE, TURQUOISE_HELM_IMBUE
    );
    ...
    private static void makeMask(Player player, boolean imbued) {
        ...
        if(World.isEco() && player.getStats().get(StatType.Crafting).currentLevel < 55) {
    ...
    static {
        ...
        for (int helm : Arrays.asList(SLAYER_HELM, SLAYER_HELM_IMBUE, RED_HELM_IMBUE, RED_SLAYER_HELM, GREEN_SLAYER_HELM, GREEN_HELM_IMBUE, BLACK_HELM_IMBUE, BLACK_SLAYER_HELM, PURPLE_HELM_IMBUE, PURPLE_SLAYER_HELM, TURQUOISE_SLAYER_HELM, TURQUOISE_HELM_IMBUE)) {
            ItemAction.registerEquipment(helm, 4, (player, item) -> {
                Slayer.check(player);
            });
            ...
            ItemAction.registerInventory(4155, "check", (player, item) -> Slayer.check(player));
            ItemAction.registerInventory(4155, "activate", (player, item) -> Slayer.check(player));
            ItemAction.registerInventory(4155, "log", (player, item) -> KillCounter.openOwnSlayer(player));
        }
    }
