...
public enum RingOfWealth {
    ...
    private static void displayFeatures(Player player, Item item) {
        if(World.isPVP()) {
            player.dialogue(new OptionsDialogue("What features would you like to toggle?",
                            new Option("Auto Collect Blood Money: " + (player.ROWAutoCollectBloodMoney ? Color.DARK_GREEN.wrap("Enabled") : Color.DARK_RED.wrap("Disabled")), () -> toggleBMCollect(player)),
    ...
    public static boolean check(Player pKiller, Item item) {
        if(!RingOfWealth.wearingRingOfWealth(pKiller))
            return false;
        if(item.getId() == 13307 && pKiller.ROWAutoCollectBloodMoney)
            return true;
        if(item.getId() == 21820 && pKiller.ROWAutoCollectEther)
            return true;
        if(item.getId() == 995 && pKiller.ROWAutoCollectGold)
            return true;
        return false;
    }
