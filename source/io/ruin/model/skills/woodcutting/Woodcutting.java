...
public class Woodcutting {
    ...
    public static void chop(Tree treeData, Player player, Supplier<Boolean> treeDeadCheck, EventConsumer treeDeadAction) {
        ...
        if(World.isPVP()) {
            player.sendMessage("Woodcutting is currently disabled on the PVP world");
            return;
        }
        if (hatchet == null) {
            ...
            while (true) {
                ...
                if (attempts % 2 == 0 && successfullyCutTree(effectiveLevel, treeData, hatchet)) {
                    if (hatchet == Hatchet.INFERNAL && (Random.rollDie(3, 1) || player.infernalAxeSpecial > 0)) {
                        ...
                        if (burning != null) {
                            player.sendMessage("The infernal axe incinerates some logs.");
                        ...
                        if (player.dragonAxeSpecial > 0 && Random.rollPercent(50)) {
                            player.sendMessage("Your axe's buff allows you to chop some additional logs!");
                            player.getInventory().add(treeData.log, 1);
    ...
    static {
        ...
        ObjectAction.register(2023, "chop down", (player, obj) -> chop(Tree.ACHEY, player, obj, 1355));
        ObjectAction.register(1751, "chop down", (player, obj) -> chop(Tree.OAK, player, obj, 1356));
        ObjectAction.register(1750, "chop down", (player, obj) -> chop(Tree.WILLOW, player, obj, 9711));
        ObjectAction.register(1756, "chop down", (player, obj) -> chop(Tree.WILLOW, player, obj, 9711));
        ObjectAction.register(1758, "chop down", (player, obj) -> chop(Tree.WILLOW, player, obj, 9711));
        ObjectAction.register(1760, "chop down", (player, obj) -> chop(Tree.WILLOW, player, obj, 9711));
        ...
        ObjectAction.register(1759, "chop down", (player, obj) -> chop(Tree.MAPLE, player, obj, 9712));
        ObjectAction.register(1753, "chop down", (player, obj) -> chop(Tree.YEW, player, obj, 9714));
        ObjectAction.register(1754, "chop down", (player, obj) -> chop(Tree.YEW, player, obj, 9714));
        ObjectAction.register(7483, "chop down", (player, obj) -> chop(Tree.MAGIC, player, obj, 9713));
