    ...
    static {
        Server.afterData.add(() -> {
            ...
            TASKS.values().forEach(task -> {
                ...
                if(task.mainSpawns == 0 && task.wildernessSpawns == 0 && !task.name.equals("Barrows Brothers") && World.debug) { // barrows brothers arent spawned in the world by default...
                    System.err.println("Slayer task \"" + task.name + "\" has 0 npcs!");
                    return;
                }
                int defaultMin, defaultMax;
                Type highest = Arrays.stream(task.type).sorted(Comparator.comparing(Type::ordinal)).findFirst().orElse(null);
