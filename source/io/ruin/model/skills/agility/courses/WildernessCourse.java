...
public class WildernessCourse {
    ...
    static {
        ...
        ObjectAction.register(23555, "open", (p, obj) -> p.startEvent(e -> {
            if (!World.isPVP() && !p.getStats().check(StatType.Agility, 52, "attempt this"))
                return;
            if(p.getCombat().getLevel() != 126 && World.isPVP()) {
                p.sendMessage("You need 126 combat in order to enter the Wilderness Course.");
                return;
            }
        ...
        Tile.get(2995, 3945, 0, true).flagUnmovable();
        /**
         * Rocks
         */
        ...
        ObjectAction.register(23640, "climb", (player, obj) -> player.startEvent(e -> {
            ...
            if (player.lastAgilityObjId == 23542) {
                if(World.isEco())
                    ...
                    int xpBoost = World.xpMultiplier;
                    player.getStats().addXp(StatType.Agility, (xpBoost == 0 || xpBoost == -1) ? 25000 : 25000 * xpBoost, false);
                }
                AgilityPet.rollForPet(player, 17000);
                PlayerCounter.WILDERNESS_COURSE.increment(player, 1);
                player.lastAgilityObjId = -1;
                if(World.isPVP()) {
                    player.getInventory().add(13307, 125);
                    int graceCount = Random.get(1, 3);
                    player.getInventory().add(11849, graceCount);
                    player.sendFilteredMessage("You find 125 blood money and " + graceCount + " mark" + (graceCount == 1 ? "" : "s") + " of grace while climbing up the rocks.");
                }
            }
            player.unlock();
        }));
