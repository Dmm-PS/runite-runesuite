        ...
        if (obj.id == chestType.getBuiltObjects()[0]) { // closed chest
            ...
            if (!player.isInOwnHouse() || killerId == player.getUserId()) {
                if (killerId == -1 || player.getUserId() != killerId) {
                    ...
                    player.animate(832);
                    player.dialogue(new ItemDialogue().one(995, "You search the chest and find " + found + " coins!"));
                    player.getInventory().addOrDrop(995, loot);
                    loot = 0;
                ...
                if (loot == MAX_AMOUNT) {
                    player.dialogue(new MessageDialogue("The chest already contains the maximum amount of coins.<br>Would you like to withdraw them?"),
                            new OptionsDialogue(new Option("Yes", () -> {
                                int added = player.getInventory().add(995, loot);
                                loot -= added;
                    ...
                    player.dialogue(new ItemDialogue().one(995, loot == 0 ? "The chest is currently empty." : "The chest contains " + NumberUtils.formatNumber(loot) + " coins."),
                            new OptionsDialogue(new Option("Add coins", () -> {
                                player.integerInput("Enter amount to add to the chest (Maximum " + NumberUtils.formatNumber(MAX_AMOUNT) + "):", amt -> {
                                    if (amt <= 0)
                                        return;
                                    amt = Math.min(MAX_AMOUNT - loot, Math.min(amt, player.getInventory().getAmount(995)));
                                    if (amt == 0)
                                        player.dialogue(new MessageDialogue("You don't have any coins!"));
                                    else {
                                        player.getInventory().remove(995, amt);
                                        loot += amt;
                                        player.dialogue(new ItemDialogue().one(995, "The chest now contains " + NumberUtils.formatNumber(loot) + " coins."));
                                    }
                                    obj.setId(chestType.getBuiltObjects()[0]);
                                });
                            }),
