...
public class Servant extends NPC {
    ...
    private void manualPayment(Player player) {
        if (player.getInventory().contains(995, getDefinition().getPayment())) {
            player.getInventory().remove(995, getDefinition().getPayment());
            save.resetActions();
    ...
    private void makePlanks(Player player, int logs, int amount) {
        ...
        if (plank == null)
            return;
        if (!player.getInventory().contains(995, plank.cost)) {
        ...
        int finalAmount = Math.min(Math.min(player.getInventory().getAmount(plank.woodId), player.getInventory().getAmount(995) / plank.cost), amount);
        if (finalAmount <= 0)
            return;
        player.getInventory().remove(plank.woodId, finalAmount);
        player.getInventory().remove(995, finalAmount * plank.cost);
        startEvent(event -> {
            state = State.FETCHING;
            setHidden(true);
            event.delay(getDefinition().getTripTime());
