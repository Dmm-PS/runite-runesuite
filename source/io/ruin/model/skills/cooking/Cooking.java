...
public class Cooking {
    ...
    private static void cook(Player player, Food food, GameObject obj, int anim, boolean fire) {
        if(World.isPVP()) {
            player.sendMessage("You can't practice the cooking skill on the PVP world!");
            return;
        }

        if (World.isEco() && !player.getStats().check(StatType.Cooking, food.levelRequirement, "cook " + food.descriptiveName))
            return;
        ...
        player.startEvent(e -> {
            ...
            while (amount-- > 0) {
                ...
                if (cookedFood(player, food, fire)) {
                    ...
                    player.sendMessage(cookingMessage(food));
                    PlayerCounter.COOKED_FOOD.increment(player, 1);
                } else {
                    rawFood.setId(food.burntID);
                    player.sendMessage("You accidentally burn the " + food.itemName + ".");
