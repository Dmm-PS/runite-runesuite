...
public enum Dart {
    ...
    Dart(int dartId, int tipId, int lvlReq, double xp) {
        ...
        this.pluralName = ItemDef.get(dartId).name.toLowerCase() + "s";
    }

    private void make(Player player, Item tipItem, Item featherItem) {
        if(World.isPVP()) {
            player.sendMessage("Fletching is currently disabled on the PVP world");
            return;
        }
        ...
        player.sendMessage("You make " + toMake + " " + pluralName + ".");
    }
