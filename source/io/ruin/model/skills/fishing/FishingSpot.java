...
public class FishingSpot {
    ...
    private FishingCatch randomCatch(int level, boolean barehand, FishingTool tool) {
        ...
        for(int i = catches.length - 1; i >= 0; i--) {
            ...
            if(World.isEco() && levelDifference < 0) {
    ...
    private void fish(Player player, NPC npc) {
        if(World.isPVP()) {
            player.sendMessage("You can't fish on the PVP world!");
            return;
        }
        boolean barehand;
        ...
        if(player.getInventory().hasId(tool.id) || hasDragonHarpoon(player)) {
            FishingCatch lowestCatch = regularCatches[0];
            if(World.isEco() && fishing.currentLevel < lowestCatch.levelReq) {
