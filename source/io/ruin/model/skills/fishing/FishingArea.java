...
public enum FishingArea {
    ...
    protected void move(NPC npc) {
        Position free = freePositions.poll();
        Position last = npc.getPosition();
        if(last == null) {
            npc.spawn(free);
            return;
        }
        freePositions.addLast(last.copy());
        npc.getMovement().teleport(free);
    }
    ...
    static {
        //Please be sure that each area has more fishing spots than it does fish!
        //Thank you :)
        ...
        RESOURCE_AREA.add(FishingSpot.CAGE, 4);
