...
public class FishingGuild {
    ...
    private static void introductionDialogue(Player player, NPC npc) {
        player.dialogue(
                ...
                new MessageDialogue("You must have level 82 Fishing " + (World.isPVP() ? " and level 126 combat" : "") + " in order to suitably impress Kylie.")
        );
    }
    ...
    static {
        ...
        ObjectAction.register(30376, "travel to platform", (player, obj) -> {
            ...
            if(World.isPVP() && player.getCombat().getLevel() != 126) {
                player.dialogue(new NPCDialogue(KYLIE_MINNOW_DOCK, "G'day, only the best toughest are allowed onto the fishing platform. You need a combat level of 126."));
                return;
            }

            int brewCount = player.getInventory().count(6685) +
                    player.getInventory().count(6687) +
                    player.getInventory().count(6689) +
                    player.getInventory().count(6691);
            if(World.isPVP() && brewCount > 13) {
                player.dialogue(new NPCDialogue(KYLIE_MINNOW_DOCK, "G'day, only the best toughest are allowed onto the fishing platform. You can't have more than 13 saradomin brews in here!"));
                return;
            }
            ...
            player.startEvent(event -> {
                ...
                event.delay(2);
                if(World.isPVP()) {
                    player.sendMessage(Color.DARK_RED.wrap("WARNING:") + " You're unable to teleport from here and players are able to attack you.");
                }
        ...
        ObjectAction.register(30377, "leave platform", (player, obj) -> player.startEvent(event -> {
            ...
            player.getPacketSender().fadeOut();
            event.delay(3);
            player.dialogue(new MessageDialogue("You board the boat and row to the " + (World.isPVP() ? "shore" : "dock") + ".").hideContinue());
            event.delay(2);
            player.getPacketSender().fadeIn();
            if(World.isPVP())
                player.getMovement().teleport(3366, 3809);
            else
        ...
        NPCAction.register(KYLIE_MINNOW_DOCK, "talk-to", (player, npc) -> {
            if(player.kylieMinnowDialogueStarted) {
                int fishingLevel = player.getStats().get(StatType.Fishing).currentLevel;
                if(fishingLevel < 82 || (World.isPVP() && player.getCombat().getLevel() != 126))
        ...
        NPCAction.register(KYLIE_MINNOW_PLATFORM, "trade", (player, npc) -> {
            String fishName = World.isPVP() ? "anglerfish" : "shark";
            ...
            int max = amount / 40;
            player.dialogue(
                    new NPCDialogue(npc, "I can give you " + (World.isPVP() ? "an anglerfish and 5 blood money" : "a shark") +  " for every 40 minnows that you give me. How many " + fishName + " would you like?"),
                    new ActionDialogue(() -> player.integerInput("How many " + fishName + " would you like? (0 - " + max + ")", amt -> {
                        ...
                        if(World.isPVP()) {
                            player.getInventory().add(13442, exchange);
                            player.getInventory().add(13307, exchange * 5);
                            player.dialogue(new MessageDialogue("You trade in " + exchange * 40 + " minnows for " + exchange + " anglerfish and " + exchange * 5 + " blood money."));
                        } else {
