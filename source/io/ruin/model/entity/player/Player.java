...
public class Player extends PlayerAttributes {
    ...
    public boolean isSeniorModerator() {
        return isGroup(PlayerGroup.SENIOR_MODERATOR) || isGroup(PlayerGroup.DISCORD_MANAGER)|| (World.isEco() && isGroup(PlayerGroup.ADMINISTRATOR));
    }
    ...
    public boolean isClassicMode() { return player.isGroup(PlayerGroup.CLASSIC_MODE); }
    ...
    public boolean isDonator() {
        return isGroup(PlayerGroup.DONATOR) || isGroup(PlayerGroup.EXTREME_DONATOR) || isGroup(PlayerGroup.GODLIKE_DONATOR)
                || isGroup(PlayerGroup.UBER_DONATOR) || isGroup(PlayerGroup.SUPER_DONATOR) || isGroup(PlayerGroup.LEGENDARY_DONATOR);
    }
    ...
    public void closeInterface(InterfaceType type) {
        //System.out.println("Interface closed: " + type);
        InterfaceHandler activeHandler = activeInterfaceHandlers[type.ordinal()];
    ...
    private SuperMysteryBox superMysteryBox;
    ...
    public void init(LoginInfo info) {
        ...
        if(position == null)
            position = new Position(3096, 3486, 0);
        else
        ...
        if(superMysteryBox == null)
            superMysteryBox = new SuperMysteryBox();
        superMysteryBox.init(this, 24, -1, -1, 510, false);
    ...
    public void start() {
         ...
         * Messages
         */
        sendMessage("Welcome to " + World.type.getWorldName() + "." + (World.isPVP() ? " On this server, you die." : ""));
        ...
        /**
         * Check to ensure player isn't stuck because of the map change
         */
        //if(player.getPosition().inBounds(EDGEVILLE) && player.getPosition().getTile().clipping != 0) {
    ...
    private void tick() {
        ...
        if(overloadTicks > 0) {
            overloadTicks--;
            if (overloadTicks % 25 == 0) {
                getStats().get(StatType.Attack).boost(overloadFlatBoost, overloadPercentBoost);
                getStats().get(StatType.Strength).boost(overloadFlatBoost, overloadPercentBoost);
                getStats().get(StatType.Defence).boost(overloadFlatBoost, overloadPercentBoost);
                getStats().get(StatType.Ranged).boost(overloadFlatBoost, overloadPercentBoost);
                getStats().get(StatType.Magic).boost(overloadFlatBoost, overloadPercentBoost);
            }
            if (overloadTicks == 50) {
                sendMessage(Color.RED.wrap("Your overload potion is about to expire."));
            } else if (overloadTicks == 0) {
                sendMessage(Color.RED.wrap("Your overload potion has expired."));
                overloadFlatBoost = 0;
                overloadPercentBoost = 0;
            }
        }
        if (prayerEnhanceTicks > 0 && prayerEnhanceFrequency > 0) {
            prayerEnhanceTicks--;
            if (prayerEnhanceTicks % prayerEnhanceFrequency == 0) {
                getStats().get(StatType.Prayer).restore(1, 0);
            }
            if (prayerEnhanceTicks == 50) {
                sendMessage(Color.RED.wrap("Your prayer enhance is about to expire."));
            } else if (prayerEnhanceTicks == 0) {
                sendMessage(Color.RED.wrap("Your prayer enhance has expired."));
                prayerEnhanceTicks = 0;
                prayerEnhanceFrequency = 0;
            }
        }
    ...
    public boolean showHitAsExperience() {
        return World.isPVP() && showHitAsExperience;
    }
