...
public class PlayerCombat extends Combat {
    ...
    public boolean canAttack(Entity target, boolean message) {
        ...
        if(target.player != null) {
            ...
            if(World.isEco() && target.npc.getCombat().getInfo().slayer_level > 0 && player.getStats().get(StatType.Slayer).currentLevel < target.npc.getCombat().getInfo().slayer_level) {
                if(message)
    ...
    private boolean hasAvaDevice() {
        ...
        return capeID == 10499 || capeID == 21898 || capeID == 13337 || capeID == 22109;
    }
    ...
    public boolean allowRetaliate(Entity attacker) {
        return target == null && player.getMovement().isAtDestination() && Config.AUTO_RETALIATE.get(player) == 0;
    }
    ...
    private void preTargetDefend(Hit hit, Entity target) {
        if(!SlayerHelm.boost(player, target, hit)
            ...
            if (player.deathEndListener == null) {
                ...
                if (useDeathStorage) {
                    ...
                    IKOD.forLostItem(player, killer, item -> {
                        ...
                        if (World.isPVP() && pKiller != null && pKiller.hideFreeItems && def.free) {
                            return;
                        }
    ...
    protected void setLevel() {
        ...
        if(level != newLevel) {
            ...
            player.getPacketSender().sendString(593, 2, "Combat Lvl: " + level);
        }
    }
    ...
    public void updateWeapon(boolean fromAutocast) {
        ...
        if(specialActive != null && (wepDef == null || wepDef.special != specialActive))
            deactivateSpecial();
        player.getPacketSender().sendString(593, 1, wepDef == null ? "Unarmed" : wepDef.name);
