...
public class Horvik {
    ...
    private final static int HORVIK = 535;
    ...
    static {
        ...
        for(ItemUpgrading upgrade : ItemUpgrading.VALUES) {
            ...
            int cost = World.isPVP() ? upgrade.bmUpgradeCost : upgrade.coinUpgradeCost;
            int currencyId = World.isPVP() ? 13307 : 995;

            ItemDef reg = ItemDef.get(upgrade.regularId);
            ItemDef up = ItemDef.get(upgrade.upgradeId);
            upgradeInfo.add(reg.name + " -> " + up.name + "  <col=800000> " + NumberUtils.formatNumber(cost));
            ItemAction.registerInventory(upgrade.upgradeId, "uncharge", (player, item) -> player.dialogue(new YesNoDialogue("Are you sure you want to do this?", "You will NOT get your " + currencyName + " back.", item, () -> {
                item.setId(upgrade.regularId);
                player.dialogue(new ItemDialogue().one(upgrade.regularId, "You release the energy from the item."));
            })));
            ItemNPCAction.register(upgrade.regularId, HORVIK, (player, item, npc) -> player.dialogue(new YesNoDialogue("Are you sure you want to do this?", "Upgrade your " + item.getDef().name + " for " + cost + " " + currencyName + "?", new Item(upgrade.upgradeId, 1), () -> {
        ...
        for(ItemBreaking brokenItem : ItemBreaking.values()) {
            ...
            ItemItemAction.register(995, brokenItem.brokenId, (player, primary, secondary) -> repairItem(player, secondary, brokenItem));
            ItemItemAction.register(13307, brokenItem.brokenId, (player, primary, secondary) -> repairItem(player, secondary, brokenItem));
    ...
    private static void repairAll(Player player, NPC npc) {
        ...
        for(Item item : player.getInventory().getItems()) {
            ...
            if(brokenItem == null)
                continue;
            totalPrice += World.isPVP() ? brokenItem.bmRepairCost : brokenItem.coinRepairCost;
