...
public enum Tanner {
    ...
    private static void tan(Player player, Tanner tanner, int amount) {
        ...
        if (!player.getInventory().contains(995, tanner.cost)) {
        ...
        player.startEvent(event -> {
            ...
            if (ItemDef.get(tanner.raw).notedId > 0) {
                if (!player.getInventory().hasRoomFor(ItemDef.get(tanner.product).notedId)) {
                    player.sendMessage("Not enough space in your inventory.");
                    return;
                }
                if (finalNoted != null) {
                    int tan = Math.min(Math.min(finalNoted.getAmount(), amount), player.getInventory().getAmount(995) / tanner.cost);
                    if (tan == 0)
                        return;
                    player.getInventory().remove(995, tanner.cost * tan);
            ...
            while (made++ < amount) {
                ...
                player.getInventory().remove(995, tanner.cost);
            }
    ...
    private static String color(Player player, int rawMaterial, int cost) {
        if ((player.getInventory().findItem(rawMaterial) == null && (ItemDef.get(rawMaterial).notedId < 1 || player.getInventory().findItem(ItemDef.get(rawMaterial).notedId) == null)) || !player.getInventory().contains(995, cost))
            return "<col=f80000>";
        return "<col=00c8f8>";
    }
